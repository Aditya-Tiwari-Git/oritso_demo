...
builder.Services.AddScoped<RoutingService>(); builder.Services.AddScoped<TicketService>(); builder.Services.AddScoped<TicketAccessService>(); builder.Services.AddScoped<ChatService>();
...
api.MapGet("/tickets/{id:int}", async (int id, HttpContext c, AppDbContext db, TicketAccessService access) =>
{
    ...
    return Results.Ok(TicketView(t, IsStaff(c)));
});
api.MapPost("/tickets", async (CreateTicketRequest r, HttpContext c, TicketService service) =>
{
    ...
    var result = await service.CreateAsync(r, UserName(c)); return Results.Created($"/api/tickets/{result.Ticket.Id}", result.Ticket);
});
...
api.MapGet("/live-support/{publicId:guid}", async (Guid publicId, HttpContext c, AppDbContext db) => { var item = await db.LiveSupportSessions.Include(x => x.Messages).FirstOrDefaultAsync(x => x.PublicId == publicId); if (item is null || (!IsStaff(c) && item.RequestedBy != UserName(c))) return Results.NotFound(); return Results.Ok(item); });
...
api.MapPost("/live-support/{publicId:guid}/accept", async (Guid publicId, HttpContext c, AppDbContext db, IHubContext<LiveSupportHub> hub) => { if (!IsStaff(c)) return Results.StatusCode(403); var item = await db.LiveSupportSessions.FirstOrDefaultAsync(x => x.PublicId == publicId); if (item is null) return Results.NotFound(); if (item.Status != "Waiting") return Results.Conflict(new { error = "Conversation is no longer waiting." }); item.Status = "Active"; item.AcceptedBy = UserName(c); item.UpdatedAt = DateTimeOffset.UtcNow; db.LiveSupportMessages.Add(new() { LiveSupportSessionId = item.Id, Sender = "System", Body = $"{UserName(c)} accepted the conversation." }); await db.SaveChangesAsync(); await hub.Clients.Group(publicId.ToString()).SendAsync("SessionUpdated", item); await hub.Clients.All.SendAsync("QueueChanged"); return Results.Ok(item); });
...
admin.MapGet("/configuration", async (AppDbContext db) => Results.Ok(new { groups = await db.AssignmentGroups.ToListAsync(), memberships = await db.AgentGroupMemberships.ToListAsync(), categories = await db.Categories.Include(x => x.Subcategories).ToListAsync(), services = await db.Services.ToListAsync(), priorities = await db.Priorities.OrderBy(x => x.Rank).ToListAsync(), statuses = await db.Statuses.OrderBy(x => x.SortOrder).ToListAsync(), rules = await db.RoutingRules.OrderBy(x => x.Order).ToListAsync(), articles = await db.KnowledgeArticles.ToListAsync() }));
admin.MapGet("/users", (CredentialStore credentials) => Results.Ok(credentials.Load().Select(x => new { x.UserName, x.DisplayName, x.Role, x.IsActive })));
admin.MapPost("/users", (UserAdminRequest request, CredentialStore credentials) => { try { var user = credentials.Upsert(request); return Results.Ok(new { user.UserName, user.DisplayName, user.Role, user.IsActive }); } catch (ArgumentException ex) { return Results.BadRequest(new { error = ex.Message }); } });
...
admin.MapDelete("/{kind}/{id:int}", async (string kind, int id, AppDbContext db) => { object? entity = kind switch { "groups" => await db.AssignmentGroups.FindAsync(id), "categories" => await db.Categories.FindAsync(id), "subcategories" => await db.Subcategories.FindAsync(id), "services" => await db.Services.FindAsync(id), "priorities" => await db.Priorities.FindAsync(id), "statuses" => await db.Statuses.FindAsync(id), "articles" => await db.KnowledgeArticles.FindAsync(id), "rules" => await db.RoutingRules.FindAsync(id), _ => null }; if (entity is null) return Results.NotFound(); db.Remove(entity); await db.SaveChangesAsync(); return Results.NoContent(); });
