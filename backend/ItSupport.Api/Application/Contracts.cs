...
public record LoginResponse(string Token, string UserName, string DisplayName, string Email, string Role, DateTimeOffset ExpiresAt);
...
public record ChatReply(Guid SessionId, string Message, object[] Articles, int? TicketId, string? TicketNumber, string? AssignmentGroup, string? CalculatedPriority, bool CanEscalate, bool CanCreateTicket, object? TicketDraft, Guid? LiveSessionId, string? LiveStatus, string Stage);
public record BotTicketRequest([Required] Guid SessionId, [Required, MaxLength(30)] string Type, [Required, MaxLength(140)] string Title, [Required, MaxLength(4000)] string Description, [Required] string Category, string Subcategory, [Required] string Service, string Impact = "Single User", string Urgency = "Medium");
...
public record PriorityRuleRequest([Required, MaxLength(100)] string Name, [MaxLength(300)] string Description, string? Keywords, string? Category, string? Service, [Required] string Priority, int Order, bool IsActive = true);
public record MembershipRequest([Required] string UserName, int[] AssignmentGroupIds);
public record UserAdminRequest([Required] string UserName, [Required] string DisplayName, [Required, EmailAddress] string Email, [Required] string Role, string? Password, bool IsActive = true);
