...
export class LiveSupport implements OnInit, OnDestroy {
  ...
  private connect() { const token = this.api.session()?.token || ''; const hubUrl = this.api.baseUrl.replace(/\/api$/, '') + '/hubs/support'; this.connection = new signalR.HubConnectionBuilder().withUrl(hubUrl, { accessTokenFactory: () => token }).withAutomaticReconnect().build(); this.connection.on('MessageReceived', m => this.active.update(x => x ? ({ ...x, messages: [...(x.messages || []), m] }) : x)); this.connection.on('SessionUpdated', () => { const id = this.active()?.publicId; if (id) this.open(id); this.load(); }); this.connection.on('SessionDeleted', () => { this.active.set(null); this.load(); }); this.connection.on('QueueChanged', () => this.load()); this.connection.start().catch(() => this.error.set('Realtime connection unavailable. Refresh to retry.')); }
}
